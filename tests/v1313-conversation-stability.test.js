...
test('message delta endpoint and duplicate suppression exist',()=>{assert.match(server,/\/api\/conversations\/:id\/messages/);assert.match(server,/getCurrentSessionMessagePage\(req\.params\.id,\{limit,afterId\}\)/);assert.match(conv,/store\.hasMessage\(chatId,id\)/);assert.match(conv,/messageAfterId/)});
