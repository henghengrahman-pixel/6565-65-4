...
async function fetchList({append=false,silent=false,reason='user'}={}){
  ...
  try{
    ...
    state.hasMore=Boolean(data.page?.hasMore);state.cursor=data.page?.nextCursor||null;const selectedBefore=!append&&selected()?store.get(selected()):null;append?store.merge(data.items||[]):store.replace(data.items||[]);if(selectedBefore&&state.hasMore&&!store.get(selectedBefore.chat_id))store.upsert(selectedBefore);renderRows();
    const cur=selected();if(cur&&!store.get(cur)&&!append&&!state.hasMore){store.select('');clearConversation('Percakapan sudah ditutup atau tidak lagi aktif.')}setConnection('LIVE');state.pollDelay=4000;
    return data;
  }catch(e){
...
function renderMessages(items,{prepend=false,hasOlder=false,append=false}={}){
  ...
  if(prepend)msgs.prepend(frag);else if(append)msgs.append(frag);else{msgs.innerHTML='';msgs.append(frag);if(!items.length)msgs.innerHTML='<div class="conversation-empty-state"><div class="empty-icon">◌</div><b>Belum ada pesan</b><span>Chat aktif ini belum memiliki pesan yang tersimpan.</span></div>'}
...
async function openChat(id){
  ...
  }catch(e){if(e.name==='AbortError')return;if(version!==state.detailVersion||selected()!==id)return;if(isClosedChatError(e)){removeClosedConversation(id);return}detailError(e.message);setConnection(navigator.onLine?'RECONNECTING':'OFFLINE')}
}
...
async function pollMessages(){const id=selected();if(!id||state.messageBusy||document.hidden)return;state.messageBusy=true;try{const r=await api(`/api/conversations/${encodeURIComponent(id)}/messages?afterId=${state.messageAfterId}&limit=50`,{timeout:8000});if(selected()!==id)return;if(r.conversation?.status==='closed'||r.conversation?.visible_in_inbox===false){store.remove(id);renderRows();clearConversation('Percakapan telah ditutup.');return}const items=r.items||[];if(items.length){state.messageAfterId=Number(items.at(-1).id||state.messageAfterId);renderMessages(items,{append:true});fetchList({silent:true,reason:'poll'})}if(r.conversation){setMode(modeOf(r.conversation),String(r.conversation.status||'ACTIVE').toUpperCase());renderSide(r.conversation,r.state||{})}setConnection('LIVE')}catch(e){if(e.name!=='AbortError'){if(isClosedChatError(e)){removeClosedConversation(id);return}setConnection(navigator.onLine?'RECONNECTING':'OFFLINE')}}finally{state.messageBusy=false}}
