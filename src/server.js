...
import { startPoller, stopPoller, pollerStatus, syncOnce, forgetChat } from './poller.js';
...
const dashboardPages=new Set(['ai-settings','conversations','archives','website-profile','knowledge','responses','ai-rules','canned-responses','telegram-cta','telegram-settings','case-management','learning','menu-penting','bekal-bot','logs','security','engage/traffic']);
for(const page of dashboardPages){
  app.get(`/${page}`,(req,res)=>res.sendFile(path.join(__dirname,page==='engage/traffic'?'../public/pages/engage-traffic.html':`../public/pages/${page}.html`)));
}
...
app.get('/api/engage/traffic',requireAdmin,async(req,res)=>{
  try{
    let cursor=null;if(req.query.cursor){try{cursor=JSON.parse(Buffer.from(String(req.query.cursor),'base64url').toString('utf8'));}catch{return res.status(400).json({ok:false,error:'INVALID_CURSOR'});}}
    const data=await listTraffic({state:req.query.state||'ALL',q:req.query.q||'',limit:req.query.limit||50,cursor});res.json({ok:true,...data});
  }catch(e){res.status(500).json({ok:false,error:'TRAFFIC_LIST_FAILED',message:e.message});}
});
app.get('/api/engage/traffic/:chatId/claim-status',requireAdmin,async(req,res)=>{try{const out=await getTrafficClaimStatus(req.params.chatId);if(!out.ok)return res.status(404).json(out);res.json(out);}catch(e){res.status(500).json({ok:false,error:'CLAIM_STATUS_FAILED',message:e.message});}});
app.post('/api/engage/traffic/:chatId/pick',requireAdmin,async(req,res)=>{
  try{
    const result=await enqueueManualTrafficClaim(req.params.chatId);
    if(!result.ok){const status=result.reason==='NOT_FOUND'?404:409;return res.status(status).json({ok:false,error:result.reason,state:result.state||null});}
    return res.status(202).json({ok:true,accepted:true,confirmed:false,...result});
  }catch(e){return res.status(500).json({ok:false,error:'TRAFFIC_PICK_FAILED',message:e.message});}
});
...
app.get('/api/conversations',requireAdmin,async(req,res)=>{
  ...
  const r=await pool.query(`SELECT c.chat_id,c.customer_name,c.customer_email,c.status,c.handling_mode,c.takeover_reason,c.takeover_at,c.last_event_at,c.last_member_event_at,c.last_ai_event_at,c.workflow_type,c.workflow_state,c.member_typing,c.member_typing_updated_at,c.created_at,c.updated_at,c.lc_inbox_rank,
    ...
    WHERE c.visible_in_inbox=true AND c.status='active' AND c.lc_active IS DISTINCT FROM false AND c.provider_activity='CHATTING'
...
app.get('/api/conversations/:id',requireAdmin,async(req,res)=>{
  try{
    ...
    const metaPromise=pool.query(`SELECT c.chat_id,c.customer_name,c.customer_email,c.status,c.handling_mode,c.handling_state,c.takeover_reason,c.takeover_at,c.last_event_at,c.last_member_event_at,c.last_ai_event_at,c.workflow_type,c.workflow_state,c.visible_in_inbox,c.provider_activity,c.lc_active,
    ...
    const itemsPromise=full?getFullContext(req.params.id,20000):getCurrentSessionMessagePage(req.params.id,{limit,beforeId});
    ...
    if(String(conversation.status||'').toLowerCase()!=='active'||conversation.lc_active!==true||conversation.visible_in_inbox!==true||String(conversation.provider_activity||'').toUpperCase()!=='CHATTING'){
      return res.status(410).json({ok:false,error:'CONVERSATION_NO_LONGER_ACTIVE',lifecycle:{status:conversation.status,providerActivity:conversation.provider_activity,visible:Boolean(conversation.visible_in_inbox)}});
    }
    const items=full?msg:msg.rows;
    let hydrationRequested=false;
    if(!full&&items.length===0){
      // Deep discovery can make an active chat visible before its durable SYNC_CHAT job has
      // persisted events. Re-arm one deduplicated job for this selected chat only; never fan
      // out get_chat across the inbox and never call the provider from every browser render.
      const summary=(await pool.query(`SELECT lc_summary FROM conversations WHERE chat_id=$1 LIMIT 1`,[req.params.id])).rows[0]?.lc_summary||{id:req.params.id};
      const job=await enqueueLiveChatIngressJob({dedupeKey:`detail-hydrate:${req.params.id}:${String(state?.session_id||state?.session_key||'current')}`,jobType:'SYNC_CHAT',chatId:req.params.id,payload:{summary},priority:100}).catch(()=>null);
      hydrationRequested=Boolean(job);
    }
    res.json({ok:true,chatId:req.params.id,conversation,items,state,full,hydrationRequested,messages:full?null:{hasOlder:msg.hasMore,beforeId:msg.beforeId,lastId:msg.lastId,limit}});
  }catch(e){await logError('dashboard','CONVERSATION_DETAIL_FAILED',e.message,{chatId:req.params.id}).catch(()=>{});res.status(500).json({ok:false,error:'CONVERSATION_DETAIL_FAILED'});}
});
app.get('/api/conversations/:id/messages',requireAdmin,async(req,res)=>{
  try{
    ...
    const [meta,msg,state]=await Promise.all([
      pool.query(`SELECT c.chat_id,c.customer_name,c.customer_email,c.status,c.handling_mode,c.handling_state,c.takeover_reason,c.takeover_at,c.last_event_at,c.last_member_event_at,c.last_ai_event_at,c.workflow_type,c.workflow_state,c.visible_in_inbox,c.provider_activity,c.lc_active FROM conversations c WHERE c.chat_id=$1 LIMIT 1`,[req.params.id]),
      getCurrentSessionMessagePage(req.params.id,{limit,afterId}),getConversationState(req.params.id)
    ]);
    const conversation=meta.rows[0];if(!conversation)return res.status(404).json({ok:false,error:'CONVERSATION_NOT_FOUND'});
    if(String(conversation.status||'').toLowerCase()!=='active'||conversation.lc_active!==true||conversation.visible_in_inbox!==true||String(conversation.provider_activity||'').toUpperCase()!=='CHATTING')return res.status(410).json({ok:false,error:'CONVERSATION_NO_LONGER_ACTIVE'});
    res.json({ok:true,chatId:req.params.id,conversation,state,items:msg.rows,nextAfterId:msg.lastId});
  }catch(e){await logError('dashboard','CONVERSATION_MESSAGES_FAILED',e.message,{chatId:req.params.id}).catch(()=>{});res.status(500).json({ok:false,error:'CONVERSATION_MESSAGES_FAILED'});}
});
