...
async function sendAndStore(livechat,chatId,text,intent,senderType='ai'){
  if(!(await db.conversationCanReply(chatId))) throw new Error('CHAT_CLOSED_NO_REPLY');
...
export async function processCustomerMessage({chatId,eventId,threadId=null,text,createdAt,livechat,attachments=[],skipInsert=false,resumeReason=''}) {
  ...
  try{
    ...
    // Fail closed before any AI work. sendAndStore repeats this gate immediately before outbound send.
    if (!(await db.conversationCanReply(chatId))) return {skipped:'provider_not_chatting'};
