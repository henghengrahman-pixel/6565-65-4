...
async function handleJob(job,livechat){
  ...
  if(type==='CLAIM_CHAT'){
    if(!chatId) throw new Error('LIVECHAT_CLAIM_CHAT_ID_REQUIRED');
    const summary=job?.payload?.summary&&typeof job.payload.summary==='object'?job.payload.summary:{id:chatId};
    const claimGeneration=Number(job?.payload?.providerGeneration ?? summary?._providerGeneration ?? 0);
    if(!(await db.claimJobIsCurrent(chatId,claimGeneration))) return {claimed:false,staleClaim:true};
    await livechat.ensureRequesterInChat(chatId);
    const detail=await livechat.getChat(chatId,{id:chatId});
    // Membership success is not enough: only a provider-confirmed chatting state may enter AI.
    const requester=String(livechat.inferredRequesterUserId||'').toLowerCase();
    const users=Array.isArray(detail?.users)?detail.users:[];
    const present=requester && users.some(u=>[u?.id,u?.email].some(v=>String(v||'').toLowerCase()===requester));
    if(present && livechat.chatState(detail).active===true && detail.is_followed==null) detail.is_followed=true;
    const activity=livechat.providerActivity(detail);
    await db.upsertConversation(detail,{visible:activity==='CHATTING',state:{...livechat.chatState(detail),providerActivity:activity,generation:claimGeneration}});
    // A newer provider observation may have won while the claim request was in flight.
    const current=await db.getConversationState(chatId);
    if(Number(current?.provider_generation||claimGeneration)>claimGeneration) return {claimed:false,staleClaim:true};
    if(activity==='CHATTING') return syncChatById(livechat,chatId,{...detail,_providerGeneration:claimGeneration});
    return {claimed:true,trafficOnly:activity};
  }
  if(type==='SYNC_CHAT'){
