...
export class LiveChatClient {
  ...
  providerActivity(summary) {
    ...
    const raw=String(st.routingStatus||'').toLowerCase();
    if(st.active===false || ['closed','ended','archived','inactive'].includes(raw)) return 'CLOSED';
    if(['queued','queue','waiting_for_agent','waiting-for-agent','unassigned','pending'].includes(raw)) return 'QUEUED';
    if(['invited','invitation'].includes(raw)) return 'INVITED';
    if(['browsing','browse'].includes(raw)) return 'BROWSING';
    if(['supervised','supervision'].includes(raw)) return 'SUPERVISED';
    if(['waiting_for_reply','waiting-for-reply'].includes(raw)) return 'WAITING_FOR_REPLY';
    // Provider activity and requester membership are intentionally independent.
    // `is_followed` answers whether this requester follows the chat; it does not turn an
    // otherwise active provider chat into a queue item. Explicit routing states above win.
    if(st.active===true) return 'CHATTING';
    return 'UNKNOWN';
  }

  isChatting(summary) { return this.providerActivity(summary)==='CHATTING'; }
  ...
  filterInbox(items) {
    ...
    if (this.inboxMode === 'all_active') return items.filter(x=>this.isChatting(x));
