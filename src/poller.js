...
let running=false, started=false, timer=null, lastTick=null, lastError=null, lastResult=null, paused=false, providerGeneration=Date.now();
function nextProviderGeneration(){providerGeneration=Math.max(providerGeneration+1,Date.now());return providerGeneration;}
...
async function processChatSnapshot(livechat,chat,summary=null,rank=null,{force=false}={}){
  ...
  const activity=livechat.providerActivity(source);
  const state={...sourceState,providerActivity:activity,generation:Number(source?._providerGeneration||0),rank};
  if(activity!=='CHATTING'){await db.upsertConversation(source,{visible:false,state});return {trafficOnly:activity};}
  ...
  const detailActivity=livechat.providerActivity(detail);
  const detailState={...livechat.chatState(detail),providerActivity:detailActivity,generation:Number(source?._providerGeneration||0),rank};
  if(detailActivity!=='CHATTING'){await db.upsertConversation(detail,{visible:false,state:detailState});return {trafficOnly:detailActivity};}
  if(detailState.active===false){ await db.markConversationEnded(chatId); summaryFingerprints.delete(chatId); return {closed:true}; }
  const detailUpsert=await db.upsertConversation(detail,{visible:true,state:{...state,...detailState,active:detailState.active ?? state.active}});
  ...
  await ensureFreshWelcomeGreeting(chatId,events,livechat);
  if(deferredMs>0) summaryFingerprints.delete(chatId); else summaryFingerprints.set(chatId,fp);
...
async function runDeepSync(livechat){
  ...
  try{
    const all=await livechat.listAllActiveChats(); const chats=all.items||[]; const seenChatIds=[];
    inventorySize=chats.length; const generation=nextProviderGeneration();
    for (const [rank, summary] of chats.entries()) {
      if(!summary?.id) continue; const chatId=String(summary.id); seenChatIds.push(chatId);
      summary._providerGeneration=generation; const rawState=livechat.chatState(summary); const activity=livechat.providerActivity(summary); const state={...rawState,providerActivity:activity,generation,rank}; await db.upsertConversation(summary,{visible:activity==='CHATTING',state});
      const fp=summaryFingerprint(summary);
      if(activity==='CHATTING') await db.enqueueLiveChatIngressJob({dedupeKey:`poll:${chatId}:${fp}`,jobType:'SYNC_CHAT',chatId,payload:{summary},priority:100}).catch(()=>{});
      else if(activity==='QUEUED') await db.requestTrafficClaim(chatId,{source:'AUTO',priority:50,summary}).catch(()=>{});
    }
    ...
    const missing=previous.filter(id=>!active.has(String(id))); const keepVisible=new Set(chats.filter(x=>livechat.providerActivity(x)==='CHATTING').map(x=>String(x.id)));
    await runConcurrent(missing,Math.min(config.lcSyncConcurrency,6),async(id)=>{
      try{
        ...
        if(st.active===false){await db.markConversationEnded(id,{reason:'DEEP_SYNC_CONFIRMED_INACTIVE'});summaryFingerprints.delete(String(id));return;}
        const activity=livechat.providerActivity(detail);
        if(activity==='CHATTING'){keepVisible.add(String(id));await db.upsertConversation(detail,{visible:true,state:{...st,providerActivity:activity,generation}});}
        else await db.upsertConversation(detail,{visible:false,state:{...st,providerActivity:activity,generation}});
      }catch(e){
        if(livechat.isChatInactiveError?.(e) || Number(e?.status)===404 || /chat\s+(?:is\s+)?(?:not\s+active|inactive)|no\s+active\s+thread/i.test(String(e?.message||''))){
          await db.markConversationEnded(id,{reason:'DEEP_SYNC_CONFIRMED_INACTIVE'}).catch(()=>{});summaryFingerprints.delete(String(id));return;
        }
    ...
    for(const id of summaryFingerprints.keys()) if(!keepVisible.has(String(id))) summaryFingerprints.delete(id);
...
export async function syncOnce(livechat,{manual=false}={}){
  ...
  try{
    ...
    const chats=rawChats; const generation=nextProviderGeneration();
    let queued=0,claims=0,trafficOnly=0,unchanged=0,enqueueErrors=0;
    ...
    for (const [rank,summary] of chats.entries()){
      ...
      try{
        summary._providerGeneration=generation; const rawState=livechat.chatState(summary); const activity=livechat.providerActivity(summary); const state={...rawState,providerActivity:activity,generation,rank};
        await db.upsertConversation(summary,{visible:activity==='CHATTING',state});
        const fp=summaryFingerprint(summary);
        const oldFp=summaryFingerprints.get(chatId);
        if(oldFp===fp){unchanged++;continue;}
        let job=null;
        if(activity==='CHATTING') job=await db.enqueueLiveChatIngressJob({dedupeKey:`poll:${chatId}:${fp}`,jobType:'SYNC_CHAT',chatId,payload:{summary},priority:100});
        else if(activity==='QUEUED'){{const claim=await db.requestTrafficClaim(chatId,{source:'AUTO',priority:50,summary});job=claim?.enqueued?{id:claim.jobId}:null;if(claim?.enqueued)claims++;}}
        else trafficOnly++;
        if(job)queued++;else if(activity==='CHATTING') unchanged++;
    ...
    lastResult={ok:true,listSource:data?._listSource||'unknown',rawChats:rawChats.length,chats:chats.length,queued,ingressEnqueued:queued,claims,trafficOnly,unchanged,enqueueErrors,processed:0,newMessages:0,inventorySize,inventoryComplete,deepSyncRunning,concurrency:config.lcSyncConcurrency,durationMs:Date.now()-syncStarted};
    await db.setIntegrationHealth('livechat_poll',{status:'OK',latencyMs:Date.now()-syncStarted,meta:{chats:chats.length,queued,ingressEnqueued:queued,unchanged,enqueueErrors}}).catch(()=>{}); return lastResult;
